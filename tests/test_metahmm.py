...
def test_forward_algorithm(data=data_disomy, logr=False):
	"""Test the implementation of the forward algorithm."""
	hmm = MetaHMM(logr=logr)
	_, _, _, karyotypes, loglik = hmm.forward_algorithm(bafs=data['baf_embryo'], lrrs=data['lrr_embryo'], mat_haps=data['mat_haps'], pat_haps=data['pat_haps'], logr=logr)
	assert loglik < 0
	if logr:

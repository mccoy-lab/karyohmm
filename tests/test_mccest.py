...
def _make_genome_ld_trio(
    n_chroms=3, m=2000, c_true=0.10, std_dev=0.12, block_len=400, seed=11
):
...
def _make_genome_ld_poc(
    n_chroms=3, m=2000, c_true=0.10, std_dev=0.12, block_len=400, freq=0.05, seed=11
):
...
def test_ci_poc_lower_fallback():
    ...
    mcc = MccEst()
    lower, x, upper = mcc.mcc_ci_poc(
        _sc_bafs, _sc_mat, _sc_freq, c_hat=0.0, std_dev=0.1
    )
    assert lower == 0.0
    assert x == 0.0
    assert upper >= 0.0
...
def test_loglik_trio_c_at_boundaries():
    ...
    for c in [0.0, 0.5]:
        assert np.isfinite(
            mcc.loglik_mcc_trio(_sc_bafs, _sc_mat, _sc_pat, c=c, std_dev=0.1)
        )
...
def test_loglik_poc_c_at_boundaries():
    ...
    for c in [0.0, 0.5]:
        assert np.isfinite(
            mcc.loglik_mcc_poc(_sc_bafs, _sc_mat, _sc_freq, c=c, std_dev=0.1)
        )
...
def test_ci_poc_nondefault_alpha():
    ...
    lo90, _, hi90 = mcc.mcc_ci_poc(
        _sc_bafs, _sc_mat, _sc_freq, c_hat=c_est, std_dev=std_dev, alpha=0.90
    )
    lo95, _, hi95 = mcc.mcc_ci_poc(
        _sc_bafs, _sc_mat, _sc_freq, c_hat=c_est, std_dev=std_dev, alpha=0.95
    )
...
def test_ci_trio_nondefault_alpha():
    ...
    lo90, _, hi90 = mcc.mcc_ci_trio(
        _sc_bafs, _sc_mat, _sc_pat, c_hat=c_est, std_dev=std_dev, alpha=0.90
    )
    lo95, _, hi95 = mcc.mcc_ci_trio(
        _sc_bafs, _sc_mat, _sc_pat, c_hat=c_est, std_dev=std_dev, alpha=0.95
    )
    assert (hi90 - lo90) <= (hi95 - lo95)
...
def test_loglik_poc_invalid_freq():
    ...
    try:
        mcc.loglik_mcc_poc(
            _sc_bafs, _sc_mat, np.full(_sc_bafs.size, 1.5), c=0.1, std_dev=0.1
        )
        assert False, "Expected AssertionError"
    except AssertionError:
        pass
...
def test_loglik_phased_trio_increases_with_contamination():
    ...
    assert mcc.loglik_mcc_phased_trio(
        bafs, mat_haps, pat_haps, pos, c=c_true
...
def test_loglik_phased_poc_increases_with_contamination():
    ...
    assert mcc.loglik_mcc_phased_poc(
        bafs, mat_haps, freqs, pos, c=c_true
...
def test_loglik_phased_trio_c_at_boundaries():
    ...
    for c in [0.0, 0.5]:
        assert np.isfinite(
            mcc.loglik_mcc_phased_trio(
                _ph_cc_bafs, _ph_mat_haps, _ph_pat_haps, _ph_pos, c=c
            )
        )
...
def test_loglik_phased_poc_c_at_boundaries():
    ...
    for c in [0.0, 0.5]:
        assert np.isfinite(
            mcc.loglik_mcc_phased_poc(
                _ph_cc_bafs, _ph_mat_haps, _ph_freqs, _ph_pos, c=c
            )
        )
...
def test_mle_phased_trio_bounds():
    ...
    mcc = MccEst()
    c_est, s_est = mcc.est_mcc_phased_trio(
        _ph_cc_bafs, _ph_mat_haps, _ph_pat_haps, _ph_pos
    )
    assert 0.0 <= c_est <= 0.5
    assert s_est > 0.0
...
def test_ci_phased_trio_wellformed():
    ...
    lower, x, upper = mcc.mcc_ci_phased_trio(
        _ph_cc_bafs, _ph_mat_haps, _ph_pat_haps, _ph_pos, c_hat=c_est, std_dev=s_est
    )
...
def test_ci_phased_poc_wellformed():
    ...
    lower, x, upper = mcc.mcc_ci_phased_poc(
        _ph_cc_bafs, _ph_mat_haps, _ph_freqs, _ph_pos, c_hat=c_est, std_dev=s_est
    )
    assert lower <= x <= upper
...
def test_ci_phased_trio_lower_fallback():
    ...
    mcc = MccEst()
    lower, x, upper = mcc.mcc_ci_phased_trio(
        _ph_cc_bafs, _ph_mat_haps, _ph_pat_haps, _ph_pos, c_hat=0.0, std_dev=0.1
    )
    assert lower == 0.0
    assert x == 0.0
    assert upper >= 0.0
...
def test_ci_phased_trio_upper_fallback():
    ...
    mcc = MccEst()
    _, x, upper = mcc.mcc_ci_phased_trio(
        _ph_cc_bafs, _ph_mat_haps, _ph_pat_haps, _ph_pos, c_hat=0.5, std_dev=0.1
    )
    assert upper == 0.5
    assert x == 0.5
...
def test_ci_phased_poc_lower_fallback():
    ...
    mcc = MccEst()
    lower, x, _ = mcc.mcc_ci_phased_poc(
        _ph_cc_bafs, _ph_mat_haps, _ph_freqs, _ph_pos, c_hat=0.0, std_dev=0.1
    )
    assert lower == 0.0
    assert x == 0.0
...
def test_ci_phased_poc_upper_fallback():
    ...
    mcc = MccEst()
    _, x, upper = mcc.mcc_ci_phased_poc(
        _ph_cc_bafs, _ph_mat_haps, _ph_freqs, _ph_pos, c_hat=0.5, std_dev=0.1
    )
    assert upper == 0.5
    assert x == 0.5
...
def test_loglik_phased_trio_invalid_c():
    ...
    try:
        mcc.loglik_mcc_phased_trio(
            _ph_cc_bafs, _ph_mat_haps, _ph_pat_haps, _ph_pos, c=0.6
        )
        assert False, "Expected AssertionError"
    except AssertionError:
        pass
...
def test_loglik_genome_poc_finite():
    ...
    mcc = MccEst()
    assert np.isfinite(
        mcc.loglik_mcc_genome_poc(
            _int_baf_list, _int_mat_list, _int_freqs_list, c=0.1, std_dev=0.1
        )
    )
...
def test_loglik_genome_trio_finite():
    ...
    mcc = MccEst()
    assert np.isfinite(
        mcc.loglik_mcc_genome_trio(
            _int_baf_list, _int_mat_list, _int_pat_list, c=0.1, std_dev=0.1
        )
    )
...
def test_loglik_genome_poc_equals_sum():
    ...
    mcc = MccEst()
    c, std_dev = 0.08, 0.12
    ll_g = mcc.loglik_mcc_genome_poc(
        _int_baf_list, _int_mat_list, _int_freqs_list, c=c, std_dev=std_dev
    )
    ll_s = sum(
...
def test_loglik_genome_trio_equals_sum():
    ...
    mcc = MccEst()
    c, std_dev = 0.08, 0.12
    ll_g = mcc.loglik_mcc_genome_trio(
        _int_baf_list, _int_mat_list, _int_pat_list, c=c, std_dev=std_dev
    )
    ll_s = sum(
...
def test_loglik_genome_phased_poc_equals_sum():
    ...
    ll_s = sum(
        ...
        for b, m, f, pos in zip(
            _int_baf_list, _int_mat_list, _int_freqs_list, _int_pos_list
        )
    )
    assert np.isclose(ll_g, ll_s)
...
def test_loglik_genome_phased_trio_equals_sum():
    ...
    ll_s = sum(
        ...
        for b, m, p, pos in zip(
            _int_baf_list, _int_mat_list, _int_pat_list, _int_pos_list
        )
    )
    assert np.isclose(ll_g, ll_s)
...
def test_est_mcc_genome_poc_bounds():
    ...
    mcc = MccEst()
    c_est, s_est = mcc.est_mcc_genome_poc(
        _int_cc_baf_list, _int_mat_list, _int_freqs_list
    )
...
def test_est_mcc_genome_trio_bounds():
    ...
    mcc = MccEst()
    c_est, s_est = mcc.est_mcc_genome_trio(
        _int_cc_baf_list, _int_mat_list, _int_pat_list
    )
    assert 0.0 <= c_est <= 0.5 and s_est > 0.0
...
def test_est_mcc_per_chrom_poc_length_and_bounds():
    ...
    mcc = MccEst()
    results = mcc.est_mcc_per_chrom_poc(
        _int_cc_baf_list, _int_mat_list, _int_freqs_list
    )
...
def test_phased_unphased_lrt_both_positive_trio():
    ...
    lrt_u = 2 * (
        mcc.loglik_mcc_genome_trio(
            _int_cc_baf_list, _int_mat_list, _int_pat_list, c=c_u, std_dev=s_u
        )
        - mcc.loglik_mcc_genome_trio(
            _int_cc_baf_list, _int_mat_list, _int_pat_list, c=0.0, std_dev=s_u
        )
    )
...
def test_phased_unphased_lrt_both_positive_poc():
    ...
    lrt_u = 2 * (
        mcc.loglik_mcc_genome_poc(
            _int_cc_baf_list, _int_mat_list, _int_freqs_list, c=c_u, std_dev=s_u
        )
        - mcc.loglik_mcc_genome_poc(
            _int_cc_baf_list, _int_mat_list, _int_freqs_list, c=0.0, std_dev=s_u
        )
    )
...
def test_genome_mle_trio_accuracy():
    ...
    mcc = MccEst()
    c_est, _ = mcc.est_mcc_genome_trio(
...
def test_realistic_ci_trio(m=10000, n=5, c=0.1):
    ...
    pgt_sim = PGTSim()
    data = [
        pgt_sim.full_ploidy_sim(m=m, mix_prop=0.0, std_dev=0.1, seed=i + 1)
        for i in range(n)
    ]
    cc_bafs = np.hstack(
        [

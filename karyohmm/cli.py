...
def read_data_np(input_fp):
    ...
    for x in ['chrom', 'pos', 'ref', 'alt', 'baf', 'lrr']:
        assert x in data
    df = pd.DataFrame({'chrom':data['chrom'].values, 'pos': data['pos'].values, 'ref': data['ref'].values, 'alt': data['alt'].values, 'baf': data['baf'].values, 'lrr': data['lrr'].values})
    return df
...
@click.option(
    "--logr", "-l", is_flag=True, required=False, default=False, show_default=True, type=bool, help="Include LRR information."
)
@click.option(
    "--viterbi", is_flag=True, required=False, default=False, show_default=True, type=bool, help="Apply a Viterbi algorithm for tracing ploidy."
)
@click.option(
    "--mode", required=True, default="Meta", type=click.Choice(['Meta', 'Euploid'], case_sensitive=False, show_default=True,
)
